...
nats stream add --subjects="claims-event-channel" --storage=file --replicas=#{MSG_QUEUE_REPLICAS}# --retention=interest --discard=new --defaults claims-event-channel

nats stream add --subjects="underwriting-event-channel" --storage=file --replicas=#{MSG_QUEUE_REPLICAS}# --retention=interest --discard=new --defaults underwriting-event-channel
...
nats consumer add --ack=explicit --deliver=all --pull --backoff=linear --backoff-steps=20 --backoff-min=5s --backoff-max=60m --max-deliver=150 --max-pending=1000 --defaults claims-event-channel underwriting_sync_rest_consumer

nats consumer add --ack=explicit --deliver=all --pull --backoff=linear --backoff-steps=20 --backoff-min=5s --backoff-max=60m --max-deliver=150 --max-pending=1000 --defaults underwriting-event-channel claims_sync_rest_consumer
